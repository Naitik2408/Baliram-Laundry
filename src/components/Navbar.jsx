      ...
      <div className='flex justify-between items-center p-2 px-2 md:p-3 md:pl-5 rounded-full border border-blue-200 bg-blue-50'>
        ...
        {location.pathname.startsWith('/order') ? (
          <div onClick={toggleCart} className=' bg-[royalblue] poppins-medium cursor-pointer text-stone-50 text-lg p-2 md:p-3 rounded-full shadow-lg shadow-stone-400 hover:scale-110 transition-all duration-150 relative'>
            ...
            <div className='absolute bg-stone-200 shadow border border-stone-300 top-[-5px] left-[-9px] text-stone-800 poppins-medium w-5 md:w-6 aspect-square flex justify-center items-center rounded-full text-[10px] md:text-[15px]'>{cartItems.length}</div>
          </div>
        ) : (
      ...
      {isCartOpen && (
        <>
          ...
          <div className='fixed inset-0 flex px-2 items-center justify-center z-50'>
            <div className='bg-white border border-stone-300 shadow-lg rounded-lg p-4 w-full md:w-[600px] h-[500px] overflow-auto flex flex-col justify-between'>
              <Cart />
              <div className='flex justify-between items-center gap-4 mt-5'>
                <div onClick={toggleCart} className='bg-red-500 shadow-lg poppins-medium shadow-stone-300 text-stone-50 px-4 md:px-5 py-3 md:py-4 rounded-full hover:scale-110 transition-all duration-300 cursor-pointer'>Close</div>
                <div onClick={handleConfirmOrder} className='bg-green-600 shadow-lg poppins-medium shadow-stone-300 text-stone-50 px-4 md:px-5 py-3 md:py-4 rounded-full hover:scale-110 transition-all duration-300 cursor-pointer'>Confirm Order</div>
              </div>
            </div>
          </div>
        </>
